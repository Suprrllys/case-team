...
| **Реализация** | 5 ролей · обучены на решениях для Axenix (Cup Moscow 2024) и Askona (Inno Case Hack 2025) · 26 методологических логик в каталоге команды (откроем в репозитории после конкурса) |
...
**Как тренировались:**
На решениях для Axenix (Cup Moscow 2024) и Askona (Inno Case Hack 2025) с кейс-чемпионатов. Решения разобраны на **26 методологических логик**; разбор каждой логики откроем в репозитории после конкурса. Дотренировка на **рабочих артефактах** — черновики, Decision Log, отвергнутые идеи — не только на финальных результатах.
...
**Частотность Core Job:** 2-6 раз в год на одного фаундера (из анализа отзывов клиентов на конкурентов методом extract-jobs-from-reviews; полный разбор — в публикации репозитория после конкурса).
...
**Главный вывод:** мы занимаем **пустую клетку** в матрице позиционирования — готовый продукт под стратегический класс задач для одного человека без IT-ресурса. Полная матрица 8 критериев × 9 конкурентов с обоснованиями — доступна по запросу к автору; будет опубликована в репозитории после конкурса.
...
Все источники цифр (Anthropic Pricing, ЦБ РФ, dreamjob.ru, fless.pro) — Приложение A0. Полная финмодель с формулами и sensitivity — доступна по запросу к автору; будет опубликована в репозитории после конкурса.
...
- Из них извлечено 26 методологических логик; разбор каждой логики — в публикации репозитория после конкурса
